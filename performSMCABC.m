...
prior_runs = zeros(1,Nparts);
particles = cell(1,Nparts);
parfor k = 1:Nparts
    ...
    while ~particle_found
        ...
        % Store particle (removed validity check)
        particle_found = true;
        particles{k}.theta = prop_theta;
        particles{k}.y = prop_y;
        particles{k}.logprior = prior.logpdf(prop_theta);
        particles{k}.S = prop_S;
...
part_Ss = getProperty(particles,'S');
part_Ds = zeros(Nparts,1);
for k = 1:Nparts
    part_Ds(k) = f_discrep( part_Ss{k} );
end
...
while looping
    ...
    save('temp_data.mat','particles','f_model', 'f_summaries', 'S_target', 'prior', 'input_options');

    
    
    %%% LOOP TERMINATION
    
    % Check termination criteria: target discrepancy reached
    if D_threshold <= options.D_target && options.terminate_D
        fprintf('SMC-ABC routine finished: target discrepancy reached.\n');
        looping = false;
    end
    
    % Check termination criteria: acceptance rate too low
    if R_rec > options.maxMCMCsteps && options.terminate_acceptance
        fprintf('SMC-ABC routine terminated: acceptance rate too low.\n');
        looping = false;
    end
    
    % Check termination criteria: distribution windowing stalled
    if options.terminate_progress
        
        % Increment counter if target discrepancy hasn't decreased
        if D_old - D_threshold < options.stall_tol
            stall_counter = stall_counter + 1;
        else
            stall_counter = 0;
        end
        
        % If stall counter has reached the specified number, terminate
        if stall_counter >= options.stall_iterations
            fprintf('SMC-ABC routine terminated: discrepancy thresholds not improving.\n');
            looping = false;
        end
